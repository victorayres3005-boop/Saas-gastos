...
import { Eye, EyeOff, MailCheck, TrendingUp } from 'lucide-react'
...
function LeftPanel() {
  return (
    <div
      ...
      style={{ background: 'linear-gradient(160deg, #1A0A00 0%, #0D0D0D 60%, #1A0D05 100%)' }}
    >
      <div className="absolute inset-0 pointer-events-none" style={{
        background: 'radial-gradient(ellipse at 30% 50%, rgba(255,107,53,0.12) 0%, transparent 65%)'
      }} />

      {/* Logo */}
      <div className="relative z-10 flex items-center gap-2.5">
        <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: '#FF6B35', boxShadow: '0 4px 16px rgba(255,107,53,0.45)' }}>
          <TrendingUp size={16} color="#fff" strokeWidth={2.5} />
        </div>
        <span style={{ fontSize: 15, fontWeight: 700, color: '#F0F0F0', letterSpacing: '-0.01em' }}>FinanceTrack</span>
      </div>

      {/* Copy */}
      <div className="relative z-10">
        <p style={{ fontSize: 11, fontWeight: 600, color: '#FF6B35', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 20 }}>
          Controle financeiro pessoal
        </p>
        <h2 style={{ fontSize: 38, fontWeight: 800, color: '#F5F5F5', lineHeight: 1.15, letterSpacing: '-0.03em', marginBottom: 20 }}>
          Pare de adivinhar.<br />
          <span style={{ color: '#FF6B35' }}>Comece a decidir.</span>
        </h2>
        <p style={{ fontSize: 15, color: '#888', lineHeight: 1.65, maxWidth: 320 }}>
          Visualize receitas, despesas e metas em um painel simples. Saiba exatamente para onde vai cada real.
        </p>
      </div>
      ...
      <div className="relative z-10 flex items-center gap-3">
        <div className="flex -space-x-2">
          {['#FF6B35', '#F5A876', '#FFD4C2'].map((c, i) => (
            <div key={i} className="w-7 h-7 rounded-full border-2 flex items-center justify-center" style={{ borderColor: '#0D0D0D', background: c }}>
              <span style={{ fontSize: 10, fontWeight: 700, color: '#fff' }}>{['J', 'M', 'A'][i]}</span>
            </div>
          ))}
        </div>
        <p style={{ fontSize: 12, color: '#555' }}>+1.000 pessoas no controle das finanças</p>
      </div>
    </div>
  )
}
